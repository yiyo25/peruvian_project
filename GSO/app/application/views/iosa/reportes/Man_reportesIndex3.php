	...
	<script>
		$(document).ready(function() {
			$("#txtAdjuntar").kendoUpload({
				async: {
					saveUrl		:	"<?php echo base_url()?>iosa/save",
					removeUrl	:	"<?php echo base_url()?>iosa/delete",
					autoUpload	: 	true
				}
			});
		});
		...
		$("#btnGrabar").click(function(event) {
			if(validator.validate()){
				...
				$.ajax({
					type 		: 	'POST',
					url 		: 	'<?php echo base_url()?>iosa/grabarReporte2',
					data 		: 	cadena,
					success		:	function(data){
										if(data!==0){
											$.ajax({
												type	:	'POST',
												url		:	'<?php echo base_url()?>iosa/vistaCodigoReporte',
												data 	: 	'codigo='+data,
