...
<script type="text/x-kendo-template" id="ventanaAlerta2">
	<p>
		<strong>Esta seguro que desea enviar a SMS</strong>
		<br/>
	</p>
	<br/>
	<center>
	...
	$(document).ready(function() {
		var grid = $('#grid').kendoGrid({
			...
			dataSource: {
				transport: {
					read: {
						url			:	'<?php echo base_url();?>iosa/listaReporte/Bandeja',
						dataType	:	'json'
					}
				},
				pageSize: 9
			...
			columns: [
				{ field: 'fecha', title: 'Fecha', filterable:false,width: 80},
				{ field: 'codigo', title: 'Codigo', width: 80},				
				{ field: 'descripcion', title: 'Descripción',filterable:false},
				{ field: 'fecha_recepcion', title: 'F. Reenvío', filterable:false,width: 90},
				{ title: 'Acción',
					command: [
						{ 	
							name	: 	'Ver', 
							click	: 	function(e){
											...
											$.ajax({
												type 	: 	'POST',
												url 	: 	'<?php echo base_url();?>iosa/viewMantenimientoReporte',
												data 	: 	cadena,
						...
						{ 	
							...
							click	: 	function(e){
										...
										$("#yesButton").click(function(){
											$.ajax({
												type 	: 	'POST',
												url 	: 	'<?php echo base_url();?>iosa/eliminarReporte',
												data 	: 	'id='+dataItem.id,
												success	:	function(data){
																if(data===''){
						...
						{ 	
							...
							click	: 	function(e){
										var dataItem = this.dataItem($(e.currentTarget).closest('tr'));
										ventana('Confirmación','#ventanaAlerta2',dataItem);
										$("#yesButton").click(function(){
											$.ajax({
												type 	: 	'POST',
												url 	: 	'<?php echo base_url();?>iosa/pasaraSMS',
												data 	: 	'id='+dataItem.id,
												success	:	function(data){
																if(data===''){
																	var grid = $('#grid').data('kendoGrid');
