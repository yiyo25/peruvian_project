...
<div class="demo-section">	
	<form class="k-content" id="frm_ManResponsable">
    	<ul>
         	<li>
            	<label for="cboProceso" class="required">Procesoss:</label>
            ...
            <li class="confirm">
            	<?php 	if($mantenimiento=='Editar'){?>
                            ...
                            <input type="hidden"id" value="<?php echo $dataResponsable->Res_ID;?>"/>
                <?php 	}else{?>
...
combobox('#cboUsuario','Seleccione el Proceso','id','usuario','<?php echo base_url()?>iosa/listaUsuario',usuario);
var cboProceso		=	$('#cboProceso').data('kendoComboBox');
var cboUsuario		=	$('#cboUsuario').data('kendoComboBox');
function usuario(){
	$.ajax({
		type	:	'POST',
		url		:	'<?php echo base_url()?>iosa/dataUsuario',
	...
	$.ajax({
		...
		success	:	function(data){
						if(data>0){
							...
							combobox('#cboSubProceso','Seleccione el Sub Proceso','id','descripcion','<?php echo base_url()?>iosa/listaSubProcesos/'+cboProceso.value(),'');
...
$(document).ready(function(e) {
	...
	$("#btnGrabar").click(function(event){
		...
		if(validator.validate()){			
			...
			$.ajax({
				type 		: 	'POST',
				url 		: 	'<?php echo base_url()?>iosa/grabarResponsable',
				data 		: 	cadena,
				success		:	function(data){
									if(data!==0){
...
<?php 	if($mantenimiento=='Editar'){?>
	<script>
		...
		if(SubProceso!=''){
			$('#cboSubProceso').attr('required','true');
			$('#divsubproceso').css('display','block');
			combobox('#cboSubProceso','Seleccione el Sub Proceso','id','descripcion','<?php echo base_url()?>iosa/listaSubProcesos/<?php echo $dataResponsable->Pro_ID;?>','');
			var cboSubProceso	=	$('#cboSubProceso').data('kendoComboBox');			
			cboSubProceso.value('<?php echo $dataResponsable->SubPro_ID;?>');
		}else{
			$('#cboSubProceso').removeAttr('required');
		}
		cboUsuario.value('<?php echo (int)$dataResponsable->idusuario;?>')
	</script>
<?php	} ?>