...
combobox('#cboSubProceso','Seleccione el Sub Proceso','id','descripcion','<?php echo base_url()?>iosa/listaSubProcesos/','');
...
$(document).ready(function(e) {
	...
	$("#btnGrabar").click(function(event){
		...
		if(validator.validate()){	
			...
			$.ajax({
				type 		: 	'POST',
				url 		: 	'<?php echo base_url()?>iosa/grabarCodigo',
				data 		: 	cadena,
				success		:	function(data){
									if(data!==0){
	...
	$("#btnEditar").click(function(event) {
		...
		if(validator.validate()){
			...
			$.ajax({
				type 		: 	'POST',
				url 		: 	'<?php echo base_url()?>iosa/editarCodigo',
				data 		: 	cadena,
				success		:	function(data){
									if(data==='1'){
...
<?php 	if($mantenimiento=='Editar'){?>
	<script>
		...
		if(SubProceso!=''){
			$('#cboSubProceso').attr('required','true');
			$('#divsubproceso').css('display','block');
			combobox('#cboSubProceso','Seleccione el Sub Proceso','id','descripcion','<?php echo base_url()?>iosa/listaSubProcesos/<?php echo $dataCodigo->Pro_ID;?>','');
			var cboSubProceso	=	$('#cboSubProceso').data('kendoComboBox');			
