...
<script>
combobox('#cboCodigo','Seleccione un Código','id','descripcion','<?php echo base_url()?>gso/listaCodigo',codigo);//
...
function codigo(){
	...
	if(id==1){
		...
		$.ajax({
			type 		: 	'POST',
			url 		: 	'<?php echo base_url()?>gso/buscarCodigo',
			data 		: 	cadena,
	...
	$.ajax({
		...
		success		:	function(data){
							if(data>0){
								...
								$('#liCboSubProceso').css('display','block');
								combobox('#cboSubProceso','Seleccione el Proceso','id','descripcion','<?php echo base_url()?>gso/listaSubProcesos/'+id,'');
							}else{
	...
	$("#btnEnviar").click(function(event) {
		if (confirm('¿Está conforme con el reporte?')) {
			...
			if(validator.validate()){
				var cadena	=	'id='+$('#id').val()+'&txtNombre='+$('#txtNombre').val()+'&cboProceso='+cboProceso.value()+'&cboSubProceso='+cboSubProceso.value()+'&cboAspecto='+cboAspecto.value()+'&cboTipoReporte='+cboTipoReporte.value()+'&txtAreaDescripcion='+$('#txtAreaDescripcion').val();
				/*alert(cadena);
				return 0;*/
				
				$.ajax({
					type 		: 	'POST',
					url 		: 	'<?php echo base_url()?>gso/enviarDuenoProceso',
					data 		: 	cadena,
					success		:	function(data){
										if(data==='1'){
