...
class Iosa extends CI_Controller{
	function __construct(){
		parent::__construct();
		$this->load->model('iosa_model');			
	}
	/*
	...
	*/
	public function index(){
		...
		$this->load->view('iosa/reportes/GRI_Reportes');
	...
	*/
	public function eliminarReporte(){
		...
		$this->iosa_model->editarReporte($_POST['id'],$arrayReporte);
	}
	
	
	public function pasaraSMS(){
	
		//error_log("ARRIBA PERU55");
		$listax = $this->iosa_model->datoReporte($_POST['id']);
	...
	public function portada(){
		$this->load->view('iosa/reportes/portada');
	}
...
/*BENITES*/
	...
	public function index2(){
		$data['descripcion']		=	$this->iosa_model->secciones(1)->Sec_descripcion;
		$data['tituloSeccion1']		=	$this->iosa_model->secciones(2)->Sec_Titulo;
		$data['descripcionSeccion1']=	$this->iosa_model->secciones(2)->Sec_descripcion;
		$this->load->view('iosa/reportes/Man_reportesIndex2',$data);
	}
...
/*CANEVARO*/
	...
	public function index3(){
		...
		$data['tituloSeccion1']		=	$this->iosa_model->secciones(2)->Sec_Titulo;
		$data['descripcionSeccion1']=	$this->iosa_model->secciones(2)->Sec_descripcion;
		$this->load->view('iosa/reportes/Man_reportesIndex3',$data);
	}
	...
	*/
	public function grabarReporte(){
		...
		while($cvalidador===1){
			...
			$cvalidador	=	$this->iosa_model->validadorCodigoReporte($codigo);
		}
		$dataReporte	=	array(
								...
								'Rep_DateLate'          =>  $_POST['txtDias']		
								
							);
		$id				=	$this->iosa_model->grabarReporte($dataReporte);
		if($id>0){
	...
	public function verReporteUsuario(){
		$data['dataReporte']		=	$this->iosa_model->datoReporte($_POST['id']);
		$data['tituloSeccion2']		=	$this->iosa_model->secciones(3)->Sec_Titulo;
		$data['descripcionSeccion2']=	$this->iosa_model->secciones(3)->Sec_descripcion;
		$data['tituloSeccion3']		=	$this->iosa_model->secciones(4)->Sec_Titulo;
		$data['descripcionSeccion3']=	$this->iosa_model->secciones(4)->Sec_descripcion;
		$data['tituloSeccion4']		=	$this->iosa_model->secciones(5)->Sec_Titulo;
		$data['descripcionSeccion4']=	$this->iosa_model->secciones(5)->Sec_descripcion;
		$data['tituloSeccion5']		=	$this->iosa_model->secciones(6)->Sec_Titulo;
		$data['descripcionSeccion5']=	$this->iosa_model->secciones(6)->Sec_descripcion;
		$data['dataRepSec']			=	$this->iosa_model->reporteSeccion($_POST['id'],array('ALL'));
		$this->load->view('iosa/reportes/VisUser_SeccionesReporte',$data);
	}
	...
	*/
	public function listaReporte($tipo){
		...
		$dataListaReporte	=	$this->iosa_model->listaReporte($estado);
		foreach($dataListaReporte as $dLR){
			...
			/*-------------------------------------------------------------------------------------*/
			$fechaInicial=$dLR->Rep_Fecha;
			$fechaActual=$dLR->Rep_DateLate;
			...
			if($fechaActual=="0000-00-00" or $fechaActual==""){
				$days =""; 	
			}
			else{
				
				$epoch=strtotime(date("d-m-Y H:i:00",time()));
				
				$epochdb=strtotime($fechaActual);		
    ...
    public function filtrarReporteProceso($tipo){
		...
		//$this->load->view('session');
		$dataListaReporte	=	$this->iosa_model->filtrarReporteProceso($estado);
		foreach($dataListaReporte as $dLR){
			$dataFecha	=	explode('-',$dLR->Rep_Fecha);
			$fecha		=	$dataFecha[2].'-'.$dataFecha[1].'-'.$dataFecha[0];
			
			$dataFechaEnvio	=	explode('-',$dLR->Rep_FechaEnvio);
			$fechaEnvio		=	$dataFechaEnvio[2].'-'.$dataFechaEnvio[1].'-'.$dataFechaEnvio[0];
    ...
    public function filtrarReporteSubProceso($tipo){
		...
		//$this->load->view('session');
		$dataListaReporte	=	$this->iosa_model->filtrarReporteSubProceso($estado);
		foreach($dataListaReporte as $dLR){
			$dataFecha	=	explode('-',$dLR->Rep_Fecha);
			$fecha		=	$dataFecha[2].'-'.$dataFecha[1].'-'.$dataFecha[0];
	...
	public function viewMantenimientoReporte(){
		...
		if($_POST['mantenimiento']==='Nuevo'){
			$this->load->view('iosa/reportes/Man_Reporte',$data);
	...
	*/
	public function listaCodigo(){
		$dataListaCodigo	=	$this->iosa_model->listaCodigo();
	...
	*/
	public function buscarCodigo(){
		$dataCodigo	=	$this->iosa_model->buscarCodigo($_POST['id']);
	...
	*/
	public function enviarDuenoProceso(){
		$dataDuenoProceso	=	$this->iosa_model->dataCorreoResponsables($_POST['cboProceso'],$_POST['cboSubProceso']);
		...
		if(count($email)===0){
			...
			$this->iosa_model->editarReporte($_POST['id'],$arrayReporte);
			$this->iosa_model->crearSeccionesReporte($_POST['id']);
	...
	*/
	function enviarCorreoDuenoProceso($email){
		...
		$config['smtp_user']= 'reportes.iosa@peruvian.pe';
		//$config['smtp_pass']= 'ven8065x';
		$config['smtp_port']= '25';
		$config['priority'] = '1';			
		$this->correo->initialize($config);
		$this->correo->from('reportes.iosa@peruvian.pe', 'Reporte IOSA - Jefe de Proceso');
		$this->correo->subject('Reporte IOSA - Jefe de Proceso');
		$this->correo->to($email);
		$msg = $this->load->view('iosa/reportes/MSJ/MSJ_ReporteSMS', $data, true);
	...
	*/
	function reportesNoAtendidos(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_ReportesNoAtendidos');
		$this->load->view('plantilla/footer');
	}
	/*
	...
	*/	
	function enviarRecordatorioRNA(){
		$dataDuenoProceso	=	$this->iosa_model->dataCorreoResponsables($_POST['idProceso'],$_POST['idSubProceso']);
		foreach($dataDuenoProceso as $dDP){
			$email[]	=	$dDP->Res_Correo;
		}
		...
		$config['priority'] = '1';			
		$this->correo->initialize($config);
		$this->correo->from('reportes.iosa@peruvian.pe', 'Reporte IOSA Recordatorio');
		$this->correo->subject('Reporte IOSA Recordatorio');
		$this->correo->to($email);
		//$this->correo->bcc('robert.paredes@peruvian.pe');		
		$msg = $this->load->view('iosa/reportes/MSJ/MSJ_RecordatorioReporte', $data, true);
		$this->correo->message($msg);
		if($dato===0){			
			echo '0';
		}else{
	...
	public function reportesSeguimiento(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_ReportesSeguimiento');
		$this->load->view('plantilla/footer');
	}
	/*
	...
	*/
	function viewSegumientoReporte(){
		...
		$data['tituloSeccion5']		=	$this->iosa_model->secciones(6)->Sec_Titulo;
		$data['descripcionSeccion5']=	$this->iosa_model->secciones(6)->Sec_descripcion;
		$data['dataRepSec']			=	$this->iosa_model->reporteSeccion($_POST['id'],array('ALL','ADM'));
		$data['tipo']				=	$_POST['tipo'];
		$this->load->view('iosa/reportes/Vis_SeccionesReporte',$data);
	}
	public function reportesTerminados(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_ReportesTerminados');
		$this->load->view('plantilla/footer');
	}
	...
	*/
	function reporteResponsables(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_ReporteResponsables');
		$this->load->view('plantilla/footer');
	}
	/*
	** Lista responsable
	*/
	function listaReporteResponsables(){		
		...
		$dataResponsable=	$this->iosa_model->listaResponsablesProceso($idPersonal);
		foreach($dataResponsable as $dR){
			$arrayReportes['ProId'][]		=	$dR->Pro_ID;
			$arrayReportes['SubProID'][]	=	$dR->SubPro_ID;
		}
		$estado			=	array(2,3,4,5);
		$dataListaReporte	=	$this->iosa_model->listaReporte($estado,$arrayReportes['ProId'],$arrayReportes['SubProID']);
		foreach($dataListaReporte as $dLR){
			$dataFecha	=	explode('-',$dLR->Rep_Fecha);
			$fecha		=	$dataFecha[2].'-'.$dataFecha[1].'-'.$dataFecha[0];
	...
	*/
	...
	function grabarSeccionesReporte(){
		...
		$this->iosa_model->editarReporte($_POST['idReporte'],$arrayReporte);
		$data	=	$this->iosa_model->grabarSeccionesReporte($_POST['id'],$arraySeccion);
		echo $data;
	}
	...
	*/
	private function enviarCorreoalEncargado(){	
		...
		$config['priority'] = '1';			
		$this->correo->initialize($config);
		$this->correo->from('reportes.iosa@peruvian.pe', 'Reporte IOSA - Encargado');
		$this->correo->subject('Reporte IOSA - Encargado');
	...
	public function enviarCorreoUsuario(){
		...
		$config['priority'] = '1';			
		$this->correo->initialize($config);
		$this->correo->from('reportes.iosa@peruvian.pe', 'Reporte IOSA');
		$this->correo->subject('Código de Reporte IOSA');
		$this->correo->to($_POST['email']);	
		$data['codigo']		=	$_POST['codigo'];
		$msg = $this->load->view('iosa/reportes/MSJ/MSJ_CodigoReporteSMS', $data, true);
		$this->correo->message($msg);
		if($dato===0){			
			echo '0';
		}else{
	...
	public function vistaCodigoReporte(){
		$data['codigo']	=	$_POST['codigo'];
		$this->load->view('iosa/reportes/Vis_CodigoReporte',$data);
	}
	...
	public function sms(){
		...
		$data['tituloSeccion1']		=	$this->iosa_model->secciones(2)->Sec_Titulo;
		$data['descripcionSeccion1']=	$this->iosa_model->secciones(2)->Sec_descripcion;
		$this->load->view('iosa/reportes/Man_reportesIndex',$data);*/
		$this->load->view('iosa/reportes/portada',$data);
	}
	/*
	...
	*/
	public function procesos(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_Procesos');
		$this->load->view('plantilla/footer');
	}
	/*
	** Mantenimiento
	*/
	public function viewMantenimientoProceso(){
		if($_POST['mantenimiento']=='Editar'){
			...
			$data['dataProceso']	=	$this->iosa_model->datoProceso($_POST['id']);		
		}else{
			$data['mantenimiento']	=	'Nuevo';
		}
		$this->load->view('iosa/reportes/Man_Procesos',$data);		
	}	
	/*
	** Lista Proceso
	*/
	public function listaProcesos(){
		$dataProcesos	=	$this->iosa_model->listaProcesos();
	...
	*/
	public function grabarProceso(){
		...
		$datosRepetidos	=	$this->iosa_model->datosRepetidos($query);
		if($datosRepetidos==='0'){
			$arrayProceso	=	array(
								'Pro_Abreviatura'	=>	$_POST['txtAbreviatura'],
								'Pro_Descripcion'	=>	$_POST['txtDescripcion']
							);
			echo $this->iosa_model->grabarProceso($arrayProceso);
		}else{
	...
	*/	
	public function editarProceso(){
		if($_POST['mantenimiento']==='Eliminar'){
			...
			echo $this->iosa_model->editarProceso($_POST['id'],$arrayProceso);
		}else{
	...
	*/
	public function subProcesos(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_subProcesos');
		$this->load->view('plantilla/footer');
	}
	/*
	** Lista Sub Proceso
	*/
	public function listaSubProcesos($id=NULL){	
		if(is_null($id)){
			$dataSubProcesos	=	$this->iosa_model->listaSubProcesos();
		}else{
			$dataSubProcesos	=	$this->iosa_model->listaSubProcesos($id);
		}		
	...
	*/
	public function viewMantenimientoSubProceso(){
		if($_POST['mantenimiento']=='Editar'){
			$data['mantenimiento']	=	'Editar';	
			$data['dataSubProceso']	=	$this->iosa_model->datoSubProceso($_POST['id']);		
		}else{
			$data['mantenimiento']	=	'Nuevo';
		}
		$this->load->view('iosa/reportes/Man_subProcesos',$data);		
	}	
	/*
	** Grabar Sub Proceso
	*/
	public function grabarSubProceso(){
		...
		if($datosRepetidos==='0'){
			...
			echo $this->iosa_model->grabarSubProceso($arraySubProceso);
		}else{
			echo 'El dato se encuentra ya registrado';
		}
	}
	...
	public function editarSubProceso(){
		if($_POST['mantenimiento']==='Eliminar'){
			$arraySubProceso	=	array(
				...
				echo $this->iosa_model->editarSubProceso($_POST['id'],$arraySubProceso);
		}else{
	...
	*/
	public function responsables(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_Responsables');
		$this->load->view('plantilla/footer');
	}
	/*
	** Lista Responsables
	*/
	public function listaResponsables(){
		$dataResponsables	=	$this->iosa_model->listaResponsables();
	...
	*/
	public function viewMantenimientoResponsables(){
		...
		$this->load->view('iosa/reportes/Man_responsables',$data);		
	}
	/*
	** Mantenimiento
	*/
	public function viewMantenimientoResponsable(){
		if($_POST['mantenimiento']=='Editar'){
			$data['mantenimiento']	=	'Editar';
			$data['dataResponsable']	=	$this->iosa_model->datoResponsable($_POST['id']);		
		}else{
			$data['mantenimiento']	=	'Nuevo';
		}
		$this->load->view('iosa/reportes/Man_responsables',$data);		
	...
	*/
	public function grabarResponsable(){
		...
		echo $this->iosa_model->grabarResponsable($arrayResponsable);
	}
	/*
	** Editar Responsable
	*/
	function editarResponsable(){
		if($_POST['mantenimiento']==='Eliminar'){
			...
			echo $this->iosa_model->editarResponsable($_POST['id'],$arrayResponsable);	
		}else{
			$arrayResponsable	=	array(
									'idusuario'	=>	$_POST['idUsuario'],
									'Pro_ID'	=>	$_POST['cboProceso'],
	...
	*/
	public function aspectos(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_Aspectos');
		$this->load->view('plantilla/footer');
	}
	public function tipoReporte(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_TipoReporte');
		$this->load->view('plantilla/footer');
	}
	public function listaSubCategoria($id){
		$dataSubCategoria	=	$this->iosa_model->listaSubCategoria($id);
	...
	public function viewMantenimientoSubCategoria(){
		if($_POST['mantenimiento']=='Editar'){
			$data['mantenimiento']		=	'Editar';
			$data['dataSubCategoria']	=	$this->iosa_model->datoSubCategoria($_POST['id']);		
		}else{
			$data['mantenimiento']		=	'Nuevo';
			$data['catID']				=	$_POST['catID'];
		}
		$this->load->view('iosa/reportes/Man_SubCategoria',$data);	
	}
	public function editarSubCategoria(){
		if($_POST['mantenimiento']==='Eliminar'){
			...
			echo $this->iosa_model->editarSubCategoria($_POST['id'],$arraySubCategoria);	
		}else{
			$arraySubCategoria	=	array(
	...
	public function grabarSubCategoria(){
		...
		echo $this->iosa_model->grabarSubCategoria($arraySubCategoria);
	}
	/*
	** Grilla Codigo
	*/
	public function codigos(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_Codigos');
		$this->load->view('plantilla/footer');
	}
	/*
	** Lista código
	*/
	public function listaCodigos(){
		$dataCodigos	=	$this->iosa_model->listaCodigos();
	...
	*/
	public function viewMantenimientoCodigo(){
		if($_POST['mantenimiento']=='Editar'){
			$data['mantenimiento']	=	'Editar';
			$data['dataCodigo']	=	$this->iosa_model->datoCodigo($_POST['id']);	
		}else{
			$data['mantenimiento']	=	'Nuevo';
		}
		$this->load->view('iosa/reportes/Man_Codigo',$data);
	}
	/*
	** Grabar Codigo
	*/
	public function grabarCodigo(){
		...
		echo $this->iosa_model->grabarCodigo($arrayCodigo);
	}
	/*
	** Editar Codigo
	*/
	function editarCodigo(){
		if($_POST['mantenimiento']==='Eliminar'){
			...
			echo $this->iosa_model->editarCodigo($_POST['id'],$arrayCodigo);
		}else{
			$arrayCodigo	=	array(
									'Cod_Abreviatura'	=>	$_POST['codigo'],
									'Cod_Descripcion'	=>	$_POST['descripcion'],
	...
	*/
	public function secciones(){
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_Secciones');
		$this->load->view('plantilla/footer');
	}
	/*
	** Lista código
	*/
	public function listaSecciones(){
		$dataSecciones	=	$this->iosa_model->listaSecciones();
	...
	*/
	public function viewMantenimientoSeccion(){
		if($_POST['mantenimiento']=='Editar'){
			$data['mantenimiento']	=	'Editar';
			$data['dataSeccion']		=	$this->iosa_model->datoSeccion($_POST['id']);
		}else{
			$data['mantenimiento']	=	'Nuevo';
		}
		$this->load->view('iosa/reportes/Man_Seccion',$data);
	}
	/*
	** Editar seccion
	*/
	public function editarSeccion(){
		...
		echo $this->iosa_model->editarSeccion($_POST['id'],$arraySeccion);	
	}
	...
	*/
	function listaUsuario(){
		$dataUsuario	=	$this->iosa_model->listaUsuario();
	...
	*/
	function dataUsuario(){
		$dataUsuario	=	$this->iosa_model->dataUsuario($_POST['id']);
	...
	*/
	function verificarCodigo(){
		$data	=	$this->iosa_model->verificarCodigo($_POST['codigo']);
	...
	*/
	public function haySubproceso(){
		$data	=	$this->iosa_model->haySubproceso($_POST['id']);
		echo count($data);
	}
	...
	public function estadistica(){
		...
		$data['dataesta']=$this->iosa_model->listaProcesosEst();
		$data['datausa']=$this->iosa_model->listaResponsables_EST();
		
		$data['datasubpro']=$this->iosa_model->listarSubProceso($_GET['proceso']);
		
		if($_GET['proceso']!=0){
		//$var_subpro_real = $_GET['subproceso'];	
		
		if($_GET['subproceso']!=0)	{
		$data['datapro']=$this->iosa_model->listaProcesoporDia_EST_sub($_GET['ano'],$_GET['proceso'],$_GET['subproceso']);
		}else{
		$data['datapro']=$this->iosa_model->listaProcesoporDia_EST($_GET['ano'],$_GET['proceso']);	
		}	
			
		}else{
		$data['datapro']=$this->iosa_model->listaProcesoporDia_EST_all($_GET['ano']);
		}
		
    	$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/EST_ReporteEstadistico',$data);
		$this->load->view('plantilla/footer');
	...
	public function recordatorio(){
		...
		$this->load->view('plantilla/header');
		$this->load->view('iosa/reportes/GRI_cron_automatica',$data);
		$this->load->view('plantilla/footer');
