<?php include "header_view.php";?>
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
				Tripulantes de vuelo <small></small>
            </h1>
            <ol class="breadcrumb">
                <li><i class="fa fa-thumb-tack"></i> Trip. Vuelo</li>
                <li class="active">
                    Listado de Trip. de Vuelo
                </li>
            </ol>
        </section>
        <!-- Main content -->
            ...
            <div class='box box-danger'>
                <div class="box-body">
                    <div id="MainContent_Div1" class="panel panel-default">
                        ...
                        <form method="post" action="<?php echo URLLOGICA?>tripulante/buscarTripulante/Vuelo" onsubmit="document.forms['buscar']['buscar'].disabled=true;" name="buscar">
                            <div class="row-fluid">
                            ...
                            <div class="row-fluid">
                                ...
                                <div class="form-group">
                                    <label for="bTIPTRIPDET_id" class="col-md-2 control-label">Tipo de Trip. de Vuelo</label>
                                    <div class="col-md-2">
                                        <select name="bTIPTRIPDET_id" id="bTIPTRIPDET_id" class="form-control input-sm js-example-basic-single" >
                                            <option value="" selected>Tipo de Trip. Vuelo</option>
                            ...
                            <div class="form-group">
                                <div class="col-md-2">
                                    <button type="button" name="limpiar" class="btn btn-danger btn-block" onclick="resetFormTripVuelo('<?php echo URLLOGICA?>tripulante/listarTripulante/Vuelo/');"> Limpiar </button>
                                </div>
                            </div>
                    ...
                    <div id="MainContent_listaPro" class="panel panel-default">
                        <div class="panel-heading clearfix" style="padding: 4px !important">
                            <span id="MainContent_tituloPro"><strong>Lista de Tripulantes de Vuelo</strong></span>
                            <?php  $Bdisabled = 0; ?>
                            <?php foreach ($this->objBotton as $botton) { ?>
                                <?php if($botton["Funcion"] == "insertTripVuelo"){ ?>
                                    <?php  $Bdisabled += 1; ?>
                                <?php } ?>
                            <?php } ?>
                            <?php if($Bdisabled > 0){ $disabled = ""; } else { $disabled = "disabled"; } ?>
                                <button type="button" class="pull-right fa fa-user-plus" data-toggle="modal" data-target="#TripVueloRegistro" <?php echo $disabled;?>></button>
                        </div>
                        <div class="area_resultado table-responsive">
                            <table id="listaTripVuelo" class="display myDataTables" cellspacing="0" cellpadding="2">
                                <thead>
                                    <tr>
                                        <th scope="col">Item</th>
                                        <th scope="col">Nombre y Apellidos</th>
                                        <th scope="col">Tipo de Trip. Vuelo</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td style="width:auto;">
                                            <?php  $Bdisabled = 0; ?>
                                            <?php foreach ($this->objBotton as $botton) { ?>
                                                <?php if($botton["Funcion"] == "listarTripVuelo"){ ?>
                                                    <?php  $Bdisabled += 1; ?>
                                                <?php } ?>
                                            <?php } ?>
                                            <?php if($Bdisabled > 0){ $disabled = ""; } else { $disabled = "disabled"; } ?>
                                                <button type="button" class="fa fa-eye" data-toggle="modal"  data-target="#TripVueloRegistro" onclick="listarDetTripVuelo('<?php echo URLLOGICA?>tripulante/listarDetTrip/',<?php echo utf8_encode($lista["TRIP_id"]);?>,'listar')" <?php echo $disabled;?>></button>
                                        </td>
                                        <td style="width:auto;">
                                            <?php  $Bdisabled = 0; ?>
                                            <?php foreach ($this->objBotton as $botton) { ?>
                                                <?php if($botton["Funcion"] == "updateTripVuelo"){ ?>
                                                    <?php  $Bdisabled += 1; ?>
                                                <?php } ?>
                                            <?php } ?>
                                            <?php if($Bdisabled > 0){ $disabled = ""; } else { $disabled = "disabled"; } ?>
                                                <button type="button" class="fa fa-pencil" data-toggle="modal" data-target="#TripVueloRegistro" onclick="listarDetTripVuelo('<?php echo URLLOGICA?>tripulante/listarDetTrip/',<?php echo utf8_encode($lista["TRIP_id"]);?>,'modificar')" <?php echo $disabled;?>></button>
                                        </td>
                                    </tr>
                                        <?php $item++; ?>
                                        <?php } ?>
    ...
    <!-- Inicio Modal-->
    <div id="TripVueloRegistro" class="modal fade" role="dialog" aria-labelledby="myModalLabel">
            ...
            <div class="modal-content">
                    ...
                    <table class="col-xs-12">
                        <tr>
                            <td style="text-align:left;">
                                <h4><strong id="titleForm">Ingresar Nuevo Trip. de Vuelo</strong></h4>
                            </td>
                            <td>
                                <button type="button" class="close btn-lg" data-dismiss="modal" onclick="limpiarFormTripulante('Vuelo');" style="background-color: red; color:white; margin:10px; padding: 0px 6px 2px 6px;text-align:right;">
                ...
                <div class="modal-body" style="padding-bottom: 10px !important">
                    <div class="row">
                        ...
                        <div class="col-md-12" style="padding-bottom: 10px;">
                            ...
                            <div class="col-md-6">
                                <select name="TIPTRIPDET_id" id="TIPTRIPDET_id" class="form-control input-sm js-example-basic-single" >
                                    <option value="" selected>Seleccione Tipo de Trip. Vuelo</option>
                                <?php foreach($this->objTipoTripulante as $lista){ ?>
                        ...
                        <div class="col-md-12" style="padding-bottom: 10px;">
                            <label for="TRIP_instructor" class="col-md-5 control-label">¿Habilitado Para Instructor?:
                            <span style="color: #FF0000"><strong>*</strong></span></label>
                ...
                <div class="modal-footer" style="padding: 10px !important">
                    <button name='insertTripulante' id='insertTripulante' type="button" class="btn btn-sm" onclick="insertTripVuelo('<?php echo URLLOGICA?>tripulante/insertTripulante/');">Grabar</button>
                    <button name='updateTripulante' id='updateTripulante' type="button" class="btn btn-sm" onclick="updateTripVuelo('<?php echo URLLOGICA?>tripulante/updateTripulante/');">Modificar</button>
                    <button name="closeTripulante" id="closeTripulante" type="button" class="btn btn-danger btn-sm" data-dismiss="modal" onclick="limpiarFormTrpiVuelo();">Cerrar</button>
                </div>
            </div>
        </div>
    </div>
