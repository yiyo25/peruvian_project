        ...
        // if page propagation is to be done through GET
            ...
            if(count($_POST)){
                ...
                foreach($_POST as $key => $val){
                    $query_lgsoft .= $coma.$key."=".$val;
                }
            }
            return htmlspecialchars(html_entity_decode($this->_properties['base_url']) . (!empty($query) ? '?' . urldecode(http_build_query($query).$query_lgsoft) : ''));
